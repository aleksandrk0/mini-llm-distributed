...
Один прогон на арендованных 2–4 GPU (~20 мин, см. [RUNBOOK](RUNBOOK_multigpu.md)):

```bash
bash run_l2.sh   # кривая 1/2/4 GPU + DDP-vs-FSDP по памяти + график bench/scaling.png
```

Эффективность масштабирования = `(tok/с при N) / (N × tok/с при 1)`. После прогона
сюда добавляется график `bench/scaling.png` и заполняется таблица:
