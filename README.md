...
`bench/scaling_compute.csv`, `scaling_comm.csv`, `optim.csv` (с колонками `params`,
`config`); `scaling.py` сам печатает `num_params`.

![Масштабирование: compute-bound vs comm-bound](bench/scaling.png)

### Масштабирование упирается в насыщение GPU, а не в число карт
Две кривые на одной установке, отличаются только размером модели и батчем.
