...
памяти**. Честно: FSDP здесь ещё и в bf16, так что выигрыш = шардирование + mixed
precision против fp32-DDP.
...
> ⚠️ **Честно о железе.** Прогон — 4× A10 на **PCIe без NVLink**, поэтому эффективность
> ~0.9, а не ~1.0: all-reduce упирается в связь (на NVLink/NVSwitch было бы ближе к
> линейному). Корректность DDP проверяется отдельно на CPU/gloo
